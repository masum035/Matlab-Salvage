...
%% ----------For unipolar-RZ signal -----------%%
[t,s] = unipolar_RZ(bits,frequency);
subplot(2,1,2);
figure(1);
plot(t,s,'LineWidth',3,color='m');
xticks([0 : 1 : length(bits)]);
hline = refline(0); % baseline y-axis=0 heighlighting 
hline.Color = 'r';
ylim([-1 1]);  % limiting y-axis
grid on; % to clearely visualize
title(["Unipolar-RZ Output Signal" num2str(bits)]);
xlabel("Time Period");
ylabel("Bits");
...
% Unipolar_Return to Zero function
function [time_series,output_signal] = unipolar_RZ(bits,bitrate)
...
for i = 0 : length(bits)-1
    if bits(i+1) == 1 % small change will be here
        output_signal(i*n+1 : (i+0.5)*n) = 1;  % when see bit=1 first half of the bit period will be 1
        output_signal((i+0.5)*n+1:(i+1)*n) = 0; % second half bit period will be 0
    else
