...
title(["Polar-RZ Output Signal" num2str(bits)]);
xlabel("Time Period");
ylabel("Bits");
...
% polar-RZ function
function [time_series,output_signal] = polar_RZ(bits, bitrate)
...
for i = 0 : length(bits) - 1
    if bits(i+1) == 1
        output_signal(i*n+1:(i+0.5)*n) = 1;
        output_signal((i+0.5)*n+1:(i+1)*n) = 0;
    else % difference is here for uniploar_RZ and Polar_RZ
        output_signal(i*n+1:(i+0.5)*n) = -1;
        output_signal((i+0.5)*n+1:(i+1)*n) = 0;
    end
end
